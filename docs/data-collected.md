...
| Online presence              | `/status.validator_info.address` + `!sync_info.catching_up` (3s) | `sentinel_validator_online{validator,address}` (value=1)       | **RPC**   | Presence gauge: emitted only when the sentinel is reporting AND the node is caught up. Series ages out of VM staleness — "missing" = "offline". |
...
| Genesis identity         | `/genesis` (every `genesis_refresh_interval`, default 12h) | `sentinel_genesis_info{chain_id,genesis_time,app_hash}`                | `/genesis` is static per chain, but the sentinel re-fetches and re-emits on `[rpc] genesis_refresh_interval` (default 12h) so VictoriaMetrics never ages the series out of its staleness window. `app_hash` is dropped as a label when gnoland reports it empty. |
...
| Per-validator voting power | `/validators[*].{address,voting_power,pub_key}` (on new block) | —                                 | **RPC**   | `sentinel_rpc_validator_set_power{address,validator,pub_key_bech32}` — one series per (validator-in-set × reporter). `pub_key_bech32` is the Ed25519 pubkey encoded as gnoland's `gpub1…` bech32 (via `pkg/gpub`); absent only when `/validators` omits the pub_key field. Dashboards dedupe across reporters via `max by (address) (...)` and join on `sentinel_validator_online{validator,address}` to compute active voting power. Cardinality bounded by validator-set size. |
...
| ------------------- | -------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Validator log lines | sentinel log collector (`internal/sentinel/logs/`) | indexed labels: `{validator, level, module}`; body is the raw JSON log line with `ts` and `level` stripped (both are promoted — `ts` to the Loki entry timestamp, `level` to a stream label — so Grafana doesn't surface them as duplicate chips) |
